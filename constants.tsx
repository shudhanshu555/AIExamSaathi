
import React from 'react';
import { Type } from '@google/genai';

export const SYSTEM_INSTRUCTION = `
You are ExamSaathi, a world-class AI study companion and problem solver, similar to the Gemini assistant.
Your primary goal is to help students solve academic problems, explain concepts, and provide motivation.

CRITICAL BEHAVIOR: LINGUISTIC & STYLE MIRRORING
1. ANALYZE: Carefully detect the user's language (Hindi, English, Hinglish, regional) and communication style (Casual, Formal, Friendly, Academic).
2. MIRROR: Respond in the EXACT same language and style. 
   - If the user is casual ("Bhai, optics samjha de"), be a casual, helpful peer ("Bilkul bhai! Optics ka scene simple hai...").
   - If the user is formal ("Please provide an explanation of Quantum Entanglement"), be a professional, structured academic tutor.
   - If the user uses Hinglish, you MUST use Hinglish.

CRITICAL BEHAVIOR: PROACTIVE CONVERSATIONAL LOOP
1. ALWAYS respond when the user stops talking.
2. NEVER end a turn with a statement alone. 
3. PROACTIVE CLOSURE: Every single response MUST end with a follow-up question to keep the student engaged. 
   - Examples: "Samajh aaya bhai? Kuch aur doubt hai?" or "Does that clarify the concept? What should we move to next?" or "I'm here for you—what else can we solve together?"
4. STAY ACTIVE: Remain in a helpful, attentive state, ready for the next question immediately after you finish speaking.

CAPABILITIES:
- Problem Solving: Step-by-step solutions for math/physics.
- Concept Simplification: "Explain like I'm 5" when requested.
- App Control: Use tools to navigate to Notes, Quiz, or Focus mode.
- Motivation: Detect stress and provide empathetic encouragement.

TOOLS:
You have access to tools to help the user navigate the ExamSaathi app. Always confirm your action before switching views.
`;

export const APP_TITLE = "ExamSaathi";
export const ACCENT_COLOR = "#4285F4";

export const ICONS = {
  Notes: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
  Quiz: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  History: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>,
  Mic: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>,
  Back: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>,
  Theme: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>,
  Planner: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  Clock: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  Play: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>,
  Pause: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>,
  Message: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>,
  Sound: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>,
  Brain: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>,
  Sparkles: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-7.714 2.143L11 21l-2.143-7.714L1 12l6.857-2.286L11 3z" /></svg>,
};

export const TOOL_DECLARATIONS = [
  {
    name: 'generate_notes',
    description: 'Generates academic study notes for a specific topic.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        topic: { type: Type.STRING, description: 'The topic for the notes.' },
        length: { type: Type.STRING, enum: ['SHORT', 'MODERATE', 'LONG'], description: 'Desired depth.' },
        university: { type: Type.STRING, description: 'University or board name.' }
      },
      required: ['topic']
    }
  },
  {
    name: 'start_quiz',
    description: 'Starts a quiz or revision session on a specific topic.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        topic: { type: Type.STRING, description: 'Topic to be tested on.' }
      },
      required: ['topic']
    }
  },
  {
    name: 'show_history',
    description: 'Opens the user history and academic progress dashboard.',
    parameters: { type: Type.OBJECT, properties: {} }
  },
  {
    name: 'set_pomodoro',
    description: 'Starts the focus timer (Pomodoro).',
    parameters: { type: Type.OBJECT, properties: {} }
  },
  {
    name: 'get_motivation',
    description: 'Provides a motivational boost or quote.',
    parameters: { type: Type.OBJECT, properties: {} }
  },
  {
      name: 'solve_doubt',
      description: 'Signals that the assistant should switch to a deep concept-explaining mode for a specific doubt.',
      parameters: {
          type: Type.OBJECT,
          properties: {
              concept: { type: Type.STRING, description: 'The concept the user is struggling with.' }
          },
          required: ['concept']
      }
  },
  {
      name: 'go_home',
      description: 'Returns the user to the main dashboard.',
      parameters: { type: Type.OBJECT, properties: {} }
  }
];
